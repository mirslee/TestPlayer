...
ssize_t recvmsg(int fd, struct msghdr *msg, int flags)
{
    ...
    DWORD dwFlags = flags;
    INT fromlen = (INT)msg->msg_namelen;
    DWORD rcvd;
    int ret;
    if (fromlen)
        ret = WSARecvFrom(fd, buf, (DWORD)msg->msg_iovlen, &rcvd, &dwFlags,
                          (sockaddr *)msg->msg_name, &fromlen, NULL, NULL);
    else
        ret = WSARecv(fd, buf, (DWORD)msg->msg_iovlen, &rcvd, &dwFlags,
                      NULL, NULL);
    free(buf);
