...
size_t mxTowc (const char *str, uint32_t */*restrict*/ pwc)
{
 ...
 */
char *mxStrcasestr (const char *haystack, const char *needle)
{
    ssize_t s;
    ...
    do
    {
        ...
        uint32_t aaa[] = {0};
        s = mxTowc (haystack, aaa/*&(uint32_t) { 0 }*/);
        haystack += s;
    }
    while (s > 0);
...
{
    MxIconv handle = mxIconvOpen ("UTF-8", charset);
    if (handle == (MxIconv)(-1))
        return NULL;
    ...
    for(unsigned mul = 4; mul < 8; mul++ )
    {
        ...
        char *tmp = out = (char *)malloc (1 + out_max);
        if (!out)
            break;

        if (mxIconv (handle, &in, &in_size, &tmp, &out_max) != (size_t)(-1)) {
            *tmp = '\0';
            break;
        }
        free(out);
...
{
    MxIconv hd = mxIconvOpen (charset, "UTF-8");
    if (hd == (MxIconv)(-1))
        return NULL;
    ...
    for (unsigned mul = 4; mul < 16; mul++)
    {
        ...
        if (mxIconv (hd, &inp, &inb, &outp, &outb) != (size_t)(-1))
        {
