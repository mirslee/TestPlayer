...
{
    const vlc_modcap_t *capa = (vlc_modcap_t*)a, *capb = (vlc_modcap_t*)b;
    return strcmp(capa->name, capb->name);
}

static void vlc_modcap_free(void *data)
{
    vlc_modcap_t *cap = (vlc_modcap_t *)data;
...
{
    const module_t *const *ma = (const module_t *const *)a, *const *mb = (const module_t *const *)b;
...
static void vlc_modcap_sort(const void *node, const VISIT which,
                            const int depth)
{
    vlc_modcap_t *const *cp = (vlc_modcap_t *const *)node, *cap = *cp;
...
static struct
{
    ...
    void *caps_tree;
    unsigned usage;
} modules = { MX_STATIC_MUTEX, NULL, NULL, 0 };
...
static int vlc_module_store(module_t *mod)
{
    ...
    vlc_modcap_t *cap = (vlc_modcap_t *)malloc(sizeof (*cap));
...
#else
int module_Map(CMxObject *obj, mx_plugin_t *plugin)
{
 ...
 */
//size_t module_LoadPlugins (CMxObject *obj)
//{
//    /*vlc_assert_locked (&modules.lock); not for static mutexes :( */
//
//    if (modules.usage == 1)
