...
{
    text_style_t *p_style = (text_style_t*)calloc( 1, sizeof(*p_style) );
    if( !p_style )
        return NULL;
...
text_style_t *text_style_Duplicate( const text_style_t *p_src )
{
    ...
    text_style_t *p_dst = (text_style_t*)calloc( 1, sizeof(*p_dst) );
    if( p_dst )
...
{
    text_segment_t* segment = (text_segment_t*)calloc( 1, sizeof(*segment) );
    if( !segment )
        return NULL;
...
unsigned int mxHtmlColor( const char *psz_value, bool* ok )
{
