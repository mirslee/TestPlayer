...
static int  Open ( CMxObject * );
static void Close( CMxObject * );
...
MxFourcc AVI_FourccGetCodec( unsigned int i_cat, MxFourcc );
static int   AVI_GetKeyFlag    ( MxFourcc , uint8_t * );
 ...
 *****************************************************************************/
static void Close ( CMxObject * p_this )
{
 ...
 *****************************************************************************/
static int Open( CMxObject * p_this )
{
    demux_t  *p_demux = (demux_t *)p_this;
    demux_sys_t     *p_sys;
    ...
    for( unsigned i = 0 ; i < i_track; i++ )
    {
        ...
        switch( p_strh->i_type )
        {
            case( AVIFOURCC_auds ):
            {
                ...
                if( tk->fmt.i_codec == MX_CODEC_VORBIS )
                {
                    tk->i_blocksize = 0; /* fix vorbis VBR decoding */
                }

                if ( tk->fmt.i_codec == MX_CODEC_MP4A )
                {
                    tk->i_samplesize = 0; /* ADTS/AAC VBR */
                }

                /* Fix broken scale/rate */
                if ( tk->fmt.i_codec == MX_CODEC_ADPCM_IMA_WAV &&
                ...
                if( tk->fmt.i_codec == MX_CODEC_MPGA &&
            ...
            case( AVIFOURCC_vids ):
            {
                if( p_vids->p_bih->biCompression == MX_FOURCC( 'D', 'X', 'S', 'B' ) )
                {
                ...
                if( p_vids->p_bih->biCompression == BI_RGB )
                {
                    ...
                    {
                        case 32:
                            tk->fmt.i_codec = MX_CODEC_RGB32;
                            Set_BMP_RGB_Masks( &tk->fmt );
                            break;
                        case 24:
                            tk->fmt.i_codec = MX_CODEC_RGB24; /* BGR (see biBitCount) */
                            Set_BMP_RGB_Masks( &tk->fmt );
                            break;
                        case 16: /* Yes it is RV15 */
                        case 15:
                            tk->fmt.i_codec = MX_CODEC_RGB15;
                            Set_BMP_RGB_Masks( &tk->fmt );
                            break;
                        case 9: /* <- TODO check that */
                            tk->fmt.i_codec = MX_CODEC_I410;
                            break;
                        case 8:
                            if ( p_vids->p_bih->biClrUsed )
                                tk->fmt.i_codec = MX_CODEC_RGBP;
                            else
                                tk->fmt.i_codec = MX_CODEC_GREY;
                            break;
                    }

                    if( tk->fmt.i_codec == MX_CODEC_RGBP )
                    {
                        const MX_BITMAPINFO *p_bi = (const MX_BITMAPINFO *) p_vids->p_bih;
                ...
                else
                {
                    ...
                    if( tk->fmt.i_codec == MX_CODEC_MP4V &&
                        !strncasecmp( (char*)&p_strh->i_handler, "XVID", 4 ) )
                    {
                        tk->fmt.i_codec           =
                        tk->fmt.i_original_fourcc = MX_FOURCC( 'X', 'V', 'I', 'D' );
                    }
                ...
                if( p_vids->i_chunk_size <= INT_MAX - sizeof(MX_BITMAPINFOHEADER) )
                {
                    int i_extra = p_vids->i_chunk_size - sizeof(MX_BITMAPINFOHEADER);
                    if( i_extra > 0 )
                    {
                        tk->fmt.p_extra = malloc( i_extra );
                        if( unlikely(tk->fmt.p_extra == NULL) )
            ...
            case( AVIFOURCC_ivas):
                ...
                tk->i_samplesize = 0;
                tk->i_dv_audio_rate = tk->fmt.i_codec == MX_CODEC_DV ? -1 : 0;
...
error:
    Close( p_this );
    return b_aborted ? MX_ETIMEOUT : MX_EGENERIC;
}
...
static int Demux_Seekable( demux_t *p_demux )
{
    ...
    if( i_track_count <= 0 )
    {
        ...
        if( i_length > 0 )
        {
            if( p_sys->i_time >= i_length )
                return MX_DEMUXER_EOF;
            return MX_DEMUXER_SUCCESS;
        }
    ...
    es_out_SetPCR( p_demux->out, MX_TS_0 + p_sys->i_time );
    p_sys->i_time += p_sys->i_read_increment;
    ...
    for( ;; )
    {
        ...
        p_frame->i_pts = MX_TS_0 + AVI_GetPTS( tk );
...
#define p_stream    p_sys->track[avi_pk.i_stream]
        ...
        else
        {
                        ...
                        AVI_GetPTS( p_stream_master ) )< 2 * CLOCK_FREQ )
            {
                ...
                p_frame->i_pts = MX_TS_0 + AVI_GetPTS( p_stream );
...
static int Seek( demux_t *p_demux, mtime_t i_date, int i_percent, bool b_accurate )
{
    ...
    if( p_sys->b_seekable )
    {
        ...
        es_out_SetPCR( p_demux->out, MX_TS_0 + p_sys->i_time );
        if( b_accurate )
            es_out_Control( p_demux->out, ES_OUT_SET_NEXT_DISPLAY_TIME, MX_TS_0 + i_date );
 ...
 ****************************************************************************/
static int AVI_GetKeyFlag( MxFourcc i_fourcc, uint8_t *p_byte )
{
    switch( i_fourcc )
    {
        case MX_CODEC_DIV1:
...
MxFourcc AVI_FourccGetCodec( unsigned int i_cat, MxFourcc i_codec )
{
    switch( i_cat )
    {
        case AUDIO_ES:
 ...
 *
 ****************************************************************************/
static void AVI_ParseStreamHeader( MxFourcc i_id,
                                   ...
                                   enum es_format_category_e *pi_type )
{
    ...
    else
    {
        *pi_number = (c1 - '0') * 10 + (c2 - '0' );
        switch( MX_TWOCC( ((uint8_t *)&i_id)[2], ((uint8_t *)&i_id)[3] ) )
        {
            case AVITWOCC_wb:
                *pi_type = AUDIO_ES;
                break;
...
static int AVI_PacketGetHeader( demux_t *p_demux, avi_packet_t *p_pk )
{
    ...
    p_pk->i_fourcc  = MX_FOURCC( p_peek[0], p_peek[1], p_peek[2], p_peek[3] );
    ...
    {
        p_pk->i_type = MX_FOURCC( p_peek[8],  p_peek[9],
                                   p_peek[10], p_peek[11] );
    }
    else
    {
