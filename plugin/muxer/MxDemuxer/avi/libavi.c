...
#include "MxCodec/MxCodecs.h"                            /* BITMAPINFOHEADER */
 ...
 *
 ****************************************************************************/
static int AVI_ChunkReadCommon( stream_t *s, avi_chunk_t *p_chk, const avi_chunk_t *p_father )
{
    ...
    if( vlc_stream_Peek( s, &p_peek, 8 ) < 8 )
    {
        if( stream_Size( s ) > 0 && (uint64_t) stream_Size( s ) > i_pos )
            MX_msgWarn( s, "can't peek at %"PRIu64, i_pos );
        else
            MX_msgDbg( s, "no more data at %"PRIu64, i_pos );
        return MX_EGENERIC;
    }
    ...
    {
        MX_msgWarn( s, "chunk %4.4s does not fit into parent %ld",
...
static int AVI_ChunkRead_list( stream_t *s, avi_chunk_t *p_container )
{
    ...
    {
        /* empty box */
        MX_msgWarn( (CMxObject*)s, "empty list chunk" );
        return MX_EGENERIC;
    }
    if( vlc_stream_Peek( s, &p_peek, 12 ) < 12 )
    {
        MX_msgWarn( (CMxObject*)s, "cannot peek while reading list chunk" );
        return MX_EGENERIC;
    }
...
#endif
    MX_msgDbg( (CMxObject*)s, "<list \'%4.4s\'>", (char*)&p_container->list.i_type );
    ...
    MX_msgDbg( (CMxObject*)s, "</list \'%4.4s\'>%x", (char*)&p_container->list.i_type, i_ret );
...
static int AVI_ChunkRead_strf( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    {
        MX_msgErr( (CMxObject*)s, "malformed avi file" );
    ...
    switch( p_strh->strh.i_type )
    {
        case( AVIFOURCC_auds ):
                 ...
                 && p_chk->common.i_chunk_size > sizeof( WAVEFORMATEX ) )
            {
                ...
                {
                    MX_msgDbg( s, "Extended header found" );
                }
            }
            else
            {
        ...
        case( AVIFOURCC_txts ):
            ...
            break;
        default:
            MX_msgWarn( (CMxObject*)s, "unknown stream type: %4.4s",
...
static int AVI_ChunkRead_strd( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    {
        MX_msgDbg( (CMxObject*)s, "Zero sized pre-JUNK section met" );
...
#ifdef AVI_DEBUG
    MX_msgDbg( (CMxObject*)s, "idx1: index entry:%d", i_count );
#endif
    AVI_READCHUNK_EXIT( MX_SUCCESS );
}
...
static int AVI_ChunkRead_indx( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    {
        MX_msgWarn( (CMxObject*)s, "unknown type/subtype index" );
    }

#ifdef AVI_DEBUG
    MX_msgDbg( (CMxObject*)s, "indx: type=%d subtype=%d entry=%d",
...
#ifdef AVI_DEBUG
    MX_msgDbg( (CMxObject*)s, "vprp: format:%d standard:%d",
...
#ifdef AVI_DEBUG
    MX_msgDbg( (CMxObject*)s, "dmlh: dwTotalFrames %d",
...
#ifdef AVI_DEBUG
    MX_msgDbg( (CMxObject*)s, "%4.4s: %s : %s",
...
int  AVI_ChunkRead( stream_t *s, avi_chunk_t *p_chk, avi_chunk_t *p_father )
{
    ...
    if( !p_chk )
    {
        MX_msgWarn( (CMxObject*)s, "cannot read null chunk" );
        return MX_EGENERIC;
    }
    ...
    {
        MX_msgWarn( (CMxObject*)s, "found null fourcc chunk (corrupted file?)" );
    ...
    MX_msgWarn( (CMxObject*)s, "unknown chunk: %4.4s (not loaded)",
            (char*)&p_chk->common.i_chunk_fourcc );
    return AVI_NextChunk( s, p_chk );
}
                     ...
                     avi_chunk_t *p_chk )
{
    ...
    {
#ifdef AVI_DEBUG
        MX_msgDbg( (CMxObject*)s, "free chunk %4.4s",
                 (char*)&p_chk->common.i_chunk_fourcc );
#endif
    ...
    {
        MX_msgWarn( (CMxObject*)s, "unknown chunk: %4.4s (not unloaded)",
                (char*)&p_chk->common.i_chunk_fourcc );
    }
...
static void AVI_ChunkDumpDebug_level( CMxObject *p_obj,
...
int AVI_ChunkReadRoot( stream_t *s, avi_chunk_t *p_root )
{
    ...
    AVI_ChunkDumpDebug_level( (CMxObject*)s, p_root, 0 );
    return MX_SUCCESS;
}
