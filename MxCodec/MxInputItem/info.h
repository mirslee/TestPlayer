...
{
    info_t *info = (info_t*)malloc(sizeof(*info));
    if (!info)
        return NULL;
...
{
    info_category_t *cat = (info_category_t*)malloc(sizeof(*cat));
    if (!cat)
        return NULL;
                                             ...
                                             int *index, const char *name)
{
    /*for (int i = 0; i < cat->i_infos; i++) {
        if (!strcmp(cat->pp_infos[i]->psz_name, name)) {
            if (index)
                *index = i;
            return cat->pp_infos[i];b
        }
    }*/
    for (int i = 0; i < cat->pp_infos.size(); i++) {
        if (!strcmp(cat->pp_infos[i]->psz_name, name)) {
            if (index)
                *index = i;
...
static inline int info_category_DeleteInfo(info_category_t *cat, const char *name)
{
    ...
    if (info_category_FindInfo(cat, &index, name)) {
        ...
        cat->pp_infos.erase(cat->pp_infos.begin()+index);
        return MX_SUCCESS;
    }
    return MX_EGENERIC;
}
